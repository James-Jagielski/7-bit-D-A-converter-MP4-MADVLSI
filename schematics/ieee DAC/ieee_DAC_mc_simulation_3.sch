...
C {devices/code.sym} 545 -655 0 0 {name="CONTROL CODE" only_toplevel=false value=".save all
.control
set wr_vecnames
set wr_singlescale
...
dowhile run <= mc_runs
  ...
  while code < 128
    ...
    wrdata ~/dev/git/7-bit-D-A-converter-MP4-MADVLSI/schematics/simulations/ieee_DAC/dac_with_external_bias_gen/test_10/ieeeDAC_\{$&run\}.txt V0 V1 V2 V3 V4 V5 V6 i(Vib) i(Viout)
    if code eq 0
      set appendwrite
...
C {/home/madvlsi/dev/git/7-bit-D-A-converter-MP4-MADVLSI/schematics/ieee DAC/ieee_simple_dac_differential_output.sym} 140 -330 0 0 {name=x1}
...
C {madvlsi/pmos3.sym} -90 -220 2 1 {name=M15
L=3
W=12
body=VDD
nf=1
...
C {madvlsi/vdd.sym} -90 -250 0 0 {name=l27 lab=VDD}
